...
```mermaid
graph TD
    A[GitHub Event: PR Opened/Updated] -->|Webhook HTTP POST| D(FastAPI Server on Railway: webhook.py)
    
    subgraph Deployed FastAPI Service
...
* **Local Webhook Tunnelling**: Ngrok (optional, via `scripts/run_tunnel.py`)
...
### 1. Prerequisites
- Python 3.12+
- Ngrok (optional, for local webhook tunnel testing)
...
### Run Real-time GitHub Webhook Pipeline Locally (Optional)
To test live pull request reviews locally using an Ngrok tunnel:
...
2. **Start Ngrok Tunnel**:
   Ensure `ngrok` library is installed (`pip install ngrok`) and run:
   ```bash
   python scripts/run_tunnel.py
   ```
3. **Configure Repository Webhook**:
   Copy the public URL generated by Ngrok (e.g. `https://<hash>.ngrok-free.app/webhook`) and add it as a webhook in your GitHub repository. Choose `application/json` as the content type and select the **"Pull requests"** event.
4. **Push & Open a PR**:
...
2. Link your GitHub repository. Select the repository and deploy using the `Dockerfile`.
3. In the **Variables** settings tab of your service, add your environment variables from `.env` (like `GROQ_API_KEY`, `GITHUB_TOKEN`, and `GITHUB_WEBHOOK_SECRET`).
4. Generate a domain under **Settings** -> **Environment** -> **Domains**. Railway will expose a public URL (e.g., `https://pr-sentinel-production.up.railway.app`).
5. Update your GitHub App Webhook URL or repository Webhook URL to point to `https://your-railway-domain.up.railway.app/webhook`.
